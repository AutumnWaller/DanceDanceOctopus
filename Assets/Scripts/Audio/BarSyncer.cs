...
public class BarSyncer : AudioSyncer
{
    ...
    {
        base.UpdateBeat();
        GetComponent<Image>().color = m_Gradient.Evaluate((transform.localScale.y - m_restScale.y) / (m_beatScale.y - m_restScale.y));
        if (m_Beat)
        {
            return;
        }
